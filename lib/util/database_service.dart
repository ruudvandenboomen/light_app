...
class DatabaseService {
  ...
  Future<int> insertPreset(Preset preset) async {
    ...
    var id = await db.insert('presets', preset.toMap());
  ...
  Future<void> updatePreset(Preset preset) async {
    ...
    var id = await db.update('presets', preset.toMap(),
        where: 'id = ?', whereArgs: [preset.id]);
    for (var light in preset.lights) {
      var lightMap = light.toMap(id);
  ...
  Future<List<Preset>> getPresets() async {
    ...
    final presets = await db.query('presets');
    var presetsResult = <Preset>[];
    for (var i = 0; i < presets.length; i++) {
      var lights = await db.query('lights',
          where: 'preset_id = ?', whereArgs: [presets[i]['id']]);
      var preset = Preset.fromDB(
          presets[i]['id'],
          presets[i]['name'],
          lights
